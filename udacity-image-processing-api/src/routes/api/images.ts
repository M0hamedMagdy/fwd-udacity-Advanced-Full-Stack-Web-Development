...
import {isImageAvailable, getAvailableImageNames, isThumbAvailable, createThumbImg, getImageFullPath} from '../../utils/utils';
...
images.get(
  ...
  async (
    ...
    if (!(await isThumbAvailable(request.query))) {
      error = await createThumbImg(request.query);
    }
    ...
    const path: null | string = await getImageFullPath(request.query);
    if (path) {
